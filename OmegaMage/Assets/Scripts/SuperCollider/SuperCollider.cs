...
public static class SuperCollider {
    ...
    public static Vector3 ClosestPointOnSurface(Collider collider, Vector3 to, float radius)
    {
        if (collider is BoxCollider)
        {
			...
			return Physics.ClosestPoint(to, (BoxCollider)collider, collider.transform.position, collider.transform.rotation);
        }
        else if (collider is SphereCollider)
        {
			//return SuperCollider.ClosestPointOnSurface((SphereCollider)collider, to);
			return Physics.ClosestPoint(to, (SphereCollider)collider, collider.transform.position, collider.transform.rotation);
        }
        else if (collider is CapsuleCollider)
        {
            //return SuperCollider.ClosestPointOnSurface((CapsuleCollider)collider, to);
			return Physics.ClosestPoint(to, (CapsuleCollider)collider, collider.transform.position, collider.transform.rotation);
			// So, who would've thunk it? Unity must have added a built-in Closest Point function between when this script was released and now.
			// Anyway, I'm gonna be using that one, since these ones here are way jankier and unreliable in comparison.
        }
    ...
    public static Vector3 ClosestPointOnSurface(CapsuleCollider collider, Vector3 to)
    {
		...
		DrawDebugPoint(collider.transform.position + upperSphere + (Vector3.up * collider.radius), Color.red, Time.deltaTime);

		DrawDebugPoint(collider.transform.position + upperSphere, Color.yellow, Time.deltaTime);
		DrawDebugPoint(collider.transform.position + lowerSphere, Color.yellow, Time.deltaTime);

		//DrawDebugPoint(collider.transform.position + upperSphere, Color.blue, Time.deltaTime);
		//DrawDebugPoint(collider.transform.position + lowerSphere, Color.blue, Time.deltaTime);
		//DrawDebugPoint(Vector3 point, Color color, float duration);
		...
		DrawDebugPoint(ct.TransformPoint(p), Color.gray, 1f);
        return ct.TransformPoint(p);

    }

	static void DrawDebugPoint(Vector3 point, Color color, float duration)
	{
